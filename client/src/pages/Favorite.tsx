...
const FavoritesPage = () => {
  ...
  const { data: highPriorityResponse, isLoading: highPriorityLoading } =
    useWorkspaceTasks(
      selectedWorkspace || undefined,
  ...
  const { data: allTasksResponse, isLoading: allTasksLoading } =
    useWorkspaceTasks(
      selectedWorkspace || undefined,
      {},
      { enabled: !!selectedWorkspace }
  ...
  const favoriteTasks = useMemo(
    () =>
      Array.isArray(favoritesResponse)
        ? favoritesResponse
        : favoritesResponse && "tasks" in favoritesResponse
        ? (favoritesResponse as { tasks: EnhancedTask[] }).tasks
        : [],
    [favoritesResponse]
  );
  ...
  const highPriorityTasks = useMemo(
    () =>
      Array.isArray(highPriorityResponse)
        ...
        : highPriorityResponse && "tasks" in highPriorityResponse
        ? (highPriorityResponse as { tasks: EnhancedTask[] }).tasks
        : [],
    [highPriorityResponse]
  );
  ...
  const allWorkspaceTasks = useMemo(
    () =>
      Array.isArray(allTasksResponse)
        ? allTasksResponse
        : allTasksResponse && "tasks" in allTasksResponse
        ? (allTasksResponse as { tasks: EnhancedTask[] }).tasks
        : [],
    [allTasksResponse]
  );
  ...
  const overdueTasks = sortedTasks.filter(
    (task) => task.favoriteReason === "overdue"
  ).length;
  const dueSoonTasks = sortedTasks.filter(
    (task) => task.favoriteReason === "dueSoon"
  ).length;
  const starredTasks = sortedTasks.filter(
    (task) => task.favoriteReason === "starred"
  ).length;
  const urgentCount = sortedTasks.filter(
    (task) => task.favoriteReason === "urgent"
  ).length;
        ...
        >
          {sortedTasks.map((task) => (
            ...
            >
              ...
              <div className="p-5">
                <h3
                  className={`font-medium text-lg mb-2 group-hover:text-primary transition-colors ${
              ...
              <div className="px-5 py-3 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                ...
                task.categoryId !== null ? (
                  <div
                    ...
                    style={{
                      backgroundColor: `${
                        (task.categoryId as { color: string; name: string })
                          .color
                      }20`,
                      color: (
                        task.categoryId as { color: string; name: string }
