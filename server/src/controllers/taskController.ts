...
import { createTaskSchema, updateTaskSchema, moveTaskSchema } from "../validators/validatorSchema";
...
export const getTasks = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { parentId, status, priority, dueDate, search } = req.query;
    ...
    const sortDirection = workspace.settings?.taskSortDirection === 'desc' ? -1 : 1;
...
export const getTask = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      let parentTask = null;
      if (task.parentId) {
        parentTask = await Task.findById(task.parentId)
          .select('_id title');
      }
    ...
    try {
      ...
      if (parentId) {
        ...
        if (String(parentTask.workspaceId) !== workspaceId) {
          return next(new HttpError("Parent task must be in the same workspace", 400));
        }
      }
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const isBeingCompleted = 
          parsedResult.data.status === 'completed' && 
        ...
        if (updateData.parentId && updateData.parentId !== task.parentId?.toString()) {
        ...
        if (
          ...
          (!task.assigneeId || updateData.assigneeId !== task.assigneeId.toString())
        ) {
        ...
        if (
          ...
          if (daysRemaining <= 7) { // Only notify if due soon
        ...
        if (
          ...
          task.ownerId.toString() !== (task.assigneeId ? task.assigneeId.toString() : null)
        ) {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (workspaceId && workspaceId !== String(task.workspaceId)) {
        ...
        if (!targetWorkspace) {
          return next(new HttpError("Target workspace not found or access denied", 404));
        }
      }
      ...
      if (parentId && parentId !== 'null') {
        ...
        if (parentTask.workspaceId.toString() !== task.workspaceId.toString() && 
            !workspaceId) {
          return next(new HttpError("Parent task is in a different workspace", 400));
        }
      }
